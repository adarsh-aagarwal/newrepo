...
const SigninPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoggingIn(true); // Show animation
    ...
    try {
      ...
      // Optional delay to show animation briefly before redirect
      setTimeout(() => {
        navigate('/');
      }, 2800);
    } catch (error) {
      console.error(error.response?.data || error.message);
      alert('Login failed. Please check your credentials.');
      setIsLoggingIn(false); // Hide animation if error
    }
  };

  // 🟦 Show animation while logging in
  if (isLoggingIn) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-white">
        <Player
          ...
          style={{ height: "200px", width: "200px" }}
        />
        <p className="text-blue-600 font-semibold text-lg mt-4">Signing you in...</p>
      </div>
    );
  }
